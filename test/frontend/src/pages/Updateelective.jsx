...
const UpdateElective = () => {
  ...
  useEffect(() => {
    const fetchElectives = async () => {
      try {
        ...
        const electivesResponse = await axios.get('https://backend-cyh2.vercel.app/api/eligible-subjects', {
          headers: { Authorization: `Bearer ${token}` },
        });

        // Fetch previously selected electives
        const selectedResponse = await axios.get('https://backend-cyh2.vercel.app/api/student/selected-electives', {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        'https://backend-cyh2.vercel.app/api/student/update-electives',
        { selectedElectives },
        { headers: { Authorization: `Bearer ${token}` } }
      );
